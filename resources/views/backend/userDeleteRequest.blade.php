        ...
        <table class="table table-bordered table-striped" id="data_table">
          ...
          @foreach($user as $row)
            @if ($row->user)
                ...
                <td><div class="notification"><img src="@if($row->user->image) @if(substr($row->user->image, 0, 4)=='http') {{$row->user->image}} @else {{asset('uploads/'.$row->user->image)}} @endif @else {{asset('assets/images/no-user.jpg')}} @endif" class="rounded-circle mr-1" alt="Image" style="height:40px;width:40px; margin-right: 10px"/><a href="{{url('dashboard/user/'.$row->user->id) }}" class="ml-md-3 mt-2" style="font-size:15px;"><b>{{$row->user->name}}</b></a></div></td>
                <td class="align-middle">
                    <div class="btn-group">
                        <a data-id="{{$row->id}}" data-bs-toggle="modal" data-bs-target="#myModal" data-toggle="modal" data-target="#myModal" style="cursor:pointer;"><button type="button" class="btn btn-danger ml-2"><span aria-hidden="true" class="fa fa-trash"></span></button></a>
                    </div>
                </td>
                {!! Form::open(['url' => 'dashboard/delete-user','method'=>'POST','class'=>'form-horizontal','id'=>'form_'.$row->id]) !!}
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-danger ToastrButton">Delete</button>
                @else
                <form action="{{url('dashboard/multi-delete-user')}}" method="post">
                  @csrf
