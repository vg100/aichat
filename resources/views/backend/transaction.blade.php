        ...
        <table class="table" id="data_table">
          ...
          @foreach($data as $row)
            <tr>
              ...
              <td>
                 @if ($row->user)
                    <a href="{{url('dashboard/user/'.$row->user->id) }}" style="font-size:15px;"><b>{{$row->user->name}}</b></a></td>
                 @else
                    <b>Deleted User</b>
                 @endif
              </td>
              ...
              @else
                ...
                @else
                <td><a class="btn btn-secondary" href="{{url('dashboard/payment-completed/'.$row->id)}}" role="button">Completed</a></td>
                @endif
              @endif
              <td>
                @if ($row->user)
                    <a class="btn btn-primary" href="{{url('dashboard/invoice-download/'.$row->payment_id)}}" role="button">Invoice</a>
                 @else
                    N/A
                 @endif
              ...
              <td>
                ...
                {!! Form::open(['url' => 'dashboard/transaction-delete','method'=>'POST','class'=>'form-horizontal','id'=>'form_'.$row->id]) !!}
