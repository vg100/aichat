        ...
        <table class="table" id="data_table">
          ...
          @foreach($data as $row)
            <tr>
              ...
              <td class="align-middle">{{(strlen($row->message) >= 20)?substr_replace($row->message, "...", 20):$row->message}} @if(strlen($row->message) >= 20)<a href="#" data-bs-toggle="modal" class="text-primary" data-value="{{$row}}" data-bs-target="#detailModal">More</a>@endif</td>
              <td class="align-middle">{{$row->created_at->format('d/m/Y')}}</td>
              <td>
                <div class="btn-group">
                  <a data-id="{{$row->id}}" data-toggle="modal" data-target="#myModal" data-bs-toggle="modal" data-bs-target="#myModal"><button type="button" class="btn btn-danger ml-2"><span aria-hidden="true" class="fa fa-trash"></span></button></a>
                </div>
                {!! Form::open(['url' => 'dashboard/contact-list/'.$row->id,'method'=>'DELETE','class'=>'form-horizontal','id'=>'form_'.$row->id]) !!}
