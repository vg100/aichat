        ...
        <table class="table" id="data_table">
          ...
          @foreach($data as $row)
            <tr>
              ...
              <td class="align-middle">
                <div class="btn-group">
                  <a class="mr-1" href="{{url('dashboard/subscription-plan/'.$row->id.'/edit') }}"><button type="button" class="btn btn-success"><span aria-hidden="true" class="fa fa-edit"></span></button></a>
                  <a data-id="{{$row->id}}" data-toggle="modal" data-bs-toggle="modal" data-bs-target="#myModal"  data-target="#myModal"><button type="button" class="btn btn-danger ml-2"><span aria-hidden="true" class="fa fa-trash"></span></button></a>
                </div>
                {!! Form::open(['url' => 'dashboard/subscription-plan/'.$row->id,'method'=>'DELETE','class'=>'form-horizontal','id'=>'form_'.$row->id]) !!}
...
<script type="text/javascript">
    ...
    $(".status").change(function(){
      ...
      $.ajax({
        type: "POST",
        url: "{{url('dashboard/subscription-plan-status')}}",
    ...
    $(".most_popular").change(function(){
      ...
      $.ajax({
        type: "POST",
        url: "{{url('dashboard/subscription-most-popular')}}",
        data: { checked : checked , id : id},
        headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
        success: function(data) {
          location.reload();
