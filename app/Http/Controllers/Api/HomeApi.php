...
class HomeApi extends Controller
{
        ...
        if($exist != null)
        {
            ...
            if($check_user != null)
            {
                ...
                if($user_data != null)
                {
                    ...
                    $date = date('y-m-d');
                    $attempts = $user->daily_reward_attempts;
                    
                    if ($date != $user->daily_reward_last_attempt) {
                        $attempts = 0;
                        $user2 = User::find($user->id);
                        $user2->daily_reward_attempts = 0;
                        $user2->daily_reward_last_attempt = $date;
                        $user2->save();
                    }
    ...
    public function rewarded_add(Request $request)
    {
        ...
        if (!empty($user))
        {
            ...
            $user->daily_reward_attempts = $user->daily_reward_attempts + 1;
            $user->daily_reward_last_attempt = $date;
            $user->save();
            
            return response()->json([
                'image_url' => PopupSetting::getPopupSetting('popup_image'),
                'link' => PopupSetting::getPopupSetting('link'),
                'cta' => PopupSetting::getPopupSetting('button_text'),
                'enabled' => PopupSetting::getPopupSetting('enabled'),
                'status' => "Success",
    ...
    public function select_language(Request $request)
    {
        ...
        if (!empty($user))
        {
            $languages = Language::select("name", "direction", "word_json")->where("name", $request->name)->get();
            $data = [];
            foreach ($languages as $language) {
                $data["word_json"] = $language->word_json;
    ...
    public function getAppAbout(Request $request)
    {
        ...
        $languages = Language::select('name')->where("status",1)->get()->toArray();
        $data = [];
