...
class SettingController extends Controller
{
    ...
    public function multiSetting()
    {
        if(Setting::getSetting('licence_active') == 0)
        {
            ...
            {
                Setting::where('key_name', 'licence_active')->update(['key_value' => 1]);
                return view('backend.multi-setting');
            }
        }
        else
        {
    ...
    public function popup_setting(Request $request)
    {
        PopupSetting::where('key_name', 'enabled')->update(['key_value' => 0]);
        foreach ($request->name as $key => $val) {
            $setting = PopupSetting::where('key_name', $key)->first();
            if (is_null($setting)) 
            {
                PopupSetting::create([
                    'key_name' => $key,
                ...
                if ($key=="popup_image" && $val && $val->isValid()) {
                    $this->upload_popup_image($val,$key);
                }
            } 
            else 
            {
                PopupSetting::where('key_name', $key)->update(['key_value' => $val]);
    ...
    public function invoice_setting(Request $request)
    {
        ...
        Cache::flush();
		return redirect(route('multi-setting.index'));
    }
    ...
    private function upload_popup_image($file,$field)
    {
        $destinationPath = './uploads';
        $extension = $file->getClientOriginalExtension();
        $fileName = Str::uuid() . '.' . $extension;
        ...
        PopupSetting::where('key_name', $field)->update(['key_value' => $fileName]);
    }
