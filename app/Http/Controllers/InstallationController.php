...
class InstallationController extends Controller
{
    ...
    public function installation(Request $request) 
    {
        ...
        if (!preg_match("/^([a-f0-9]{8})-(([a-f0-9]{4})-){3}([a-f0-9]{12})$/i", $code)) {
            return response()->json(['status'=> 'fail','error' => "Invalid keyCode"]);
        }
    
        $ch = curl_init();
        curl_setopt_array($ch, array(
            CURLOPT_URL => "https://api.googles.com/v3/market/author/sale?code=".$code,
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_TIMEOUT => 20,
            CURLOPT_HTTPHEADER => array(
                "Authorization: Bearer ".$personalToken,
                "User-Agent: keyCode verification script"
            )
        ));
        ...
        //  dd($responseCode);
        switch ($responseCode) {
            case 404: return response()->json(['status'=> 'fail','error' => "Invalid keyCode"]);
        ...
        if($responseCode == 200)
        {
            if($body->buyer == $userName)
            {
                ...
                return response()->json(['status'=> 'success','msg'=>'keyCode Validation Success!','purchase_code' => $request->purchase_code,'url' =>$url,'userName' => $userName]);
            }
            else
            {
    ...
    public function database_setup_post(Request $request)
    {
        ...
        // This step is important - requests with incorrect formats can be blocked!
        if (!preg_match("/^([a-f0-9]{8})-(([a-f0-9]{4})-){3}([a-f0-9]{12})$/i", $code)) {
            return redirect()->back()->with(['message' => "Invalid keyCode"]);
        }
        ...
        //  dd($responseCode);
        switch ($responseCode) {
            case 404: return redirect()->back()->with(['message' => "Invalid keyCode"]);
...
';
    ...
    public function migration()
    {
        ...
        if ($database == 'true') 
        {
            file_put_contents(storage_path('installed'), 'ai_builder2');
            return redirect('/');
        } else {
            abort(404);
        }
